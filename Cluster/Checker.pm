package Engine::Cluster::Checker;
use Moose::Role;
...
method rerun {
	...
	#### CHECK INPUTS
	die "Engine::Cluster::Checker::rerun    min not defined\n" if not defined $min;
	die "Engine::Cluster::Checker::rerun    max not defined\n" if not defined $max;
	die "Engine::Cluster::Checker::rerun    outputdir not defined\n" if not defined $outputdir;
	die "Engine::Cluster::Checker::rerun    replicates not defined\n" if not defined $replicates;
	die "Engine::Cluster::Checker::rerun    referencedir not defined\n" if not defined $referencedir;
	die "Engine::Cluster::Checker::rerun    label not defined\n" if not defined $label;
 ...
 and exit if not -f $splitfile;
		...
		open(FILE,$logfile) or die "Engine::Cluster::Checker::rerun    Can't open logfile: $logfile\n";
		my @lines = <FILE>;
		close(FILE) or die "Engine::Cluster::Checker::rerun    Can't close logfile: $logfile\n";
...
method generateBatchJobs ( $outputdir, $referencefiles, $splitfiles, $label ) {
...
method checkFilesizes ( $logfile, $min, $max ) {
	my @checkfiles;
	my $counter = 0;
	my $fh;
	$/ = undef;
...
method checkStatus ( $jobs ) {
	$self->logDebug("Engine::Cluster::Checker::checkStatus(jobs)");
	$self->logDebug("no. jobs: " . scalar(@$jobs));
...
method batchCheckfile ( $label, $outputdir ) {
...
method checkOutput ( $lockfile, $checkfile ) {
...
method printCheckLog ( $logfile, $jobs, $label, $outdir ) {
