...
# $self->logDebug( "DeBUG EXIT" ) and exit;
  ...
  foreach my $stageparameter ( @$stageparameters ) {
    $stageparameter->{value} = $self->replaceTags( $stageparameter->{value}, $userhome, $fileroot, $projectname, $workflowname, $installdir );
  }
  ...
  #### LINUX/GIT BASH VERSION OF time 
 ...
 -f "%Uuser %Ssystem %Eelapsed %PCPU (%Xtext+%Ddata %Mmax)k"};
  ...
  if ( defined $prescript and $prescript ne "" ) {
    $prescript  =  $self->replaceTags( $prescript, $userhome, $fileroot, $projectname, $workflowname, $installdir );
  ...
  $exports  =  $self->replaceTags( $exports, $userhome, $fileroot, $projectname, $workflowname, $installdir );
...
method replaceTags ( $string, $userhome, $fileroot, $projectname, $workflowname, $installdir ) {
  $self->logCaller();
